...
def test_upstream_version_is_the_pinned_one():
    ...
    assert clipper2.__version__ == "0.1.0"
...
def test_simplify_and_rdp():
    path = [(0, 0), (5, 0), (10, 0), (10, 10), (0, 10)]
    assert clipper2.simplify_path(path, 1.0).tolist() == [[0, 0], [10, 0], [10, 10], [0, 10]]
    assert len(clipper2.simplify_paths([path], 1.0)) == 1
    assert clipper2.ramer_douglas_peucker(path, 1.0).tolist() == [
        [0, 0],
        [10, 0],
        [10, 10],
        [0, 10],
    ]
    assert isinstance(clipper2.ramer_douglas_peucker([path], 1.0), list)
